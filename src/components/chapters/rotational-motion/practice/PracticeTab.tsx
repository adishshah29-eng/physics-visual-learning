...
const RotationalPractice: React.FC = () => {
  ...
  const handleNext = () => {
    setSelected(null);
    setIndex(p => Math.min(p + 1, filtered.length - 1));
  };
        ...
        <div className="space-y-3">
          ...
          <div className="bg-black/40 border border-border
            ...
            {current.solution.map((step, i) => (
              <div key={i} className="text-sm text-foreground font-mono">
                {step}
              </div>
            ))}
          </div>
...
export default RotationalPractice;
