...
const importData = async () => {
  console.log("Reading Chemistry JSON file...");
  const jsonPath = path.join(__dirname, '../mcqs/chemistry_clean.json');
  ...
  const formattedQuestions = batch.map(q => {
    ...
    return {
      id: q.id,
      exam: 'jee-main',
      subject: 'chemistry', // Force chemistry (overrides scrape scraper bugs)
  ...
  console.log(`Writing ${formattedQuestions.length} parsed records to mcqs/mapped_chemistry.json in case you want to upload via Supabase Dashboard...`);
  fs.writeFileSync(path.join(__dirname, '../mcqs/mapped_chemistry.json'), JSON.stringify(formattedQuestions, null, 2));
